...
mainBlock
   : eol? MAIN eol typeDeclarations? mainStatements? END MAIN eol | eol
   | eol? typeDeclarations? mainStatements
   ;
...
typeDeclaration
   : DEF VAR variableDeclaration 
   | DEFINE VARIABLE variableDeclaration 
   ;

variableDeclaration
   : constantIdentifier AS type eol
   ;
...
assignmentStatement
   : ASSIGN? variable EQUAL expression (COMMA expression)*   
   ;
...
forEachStatement
   : FOR EACH expression (USING variableList)? (INTO variableList)? (WITH REOPTIMIZATION)? (WHERE expression)? NOLOCK? NOWAIT? COLON codeBlock END eol
   ;
